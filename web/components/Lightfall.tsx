...
export type LightfallProps = {
  ...
  /** Render falling streaks as pure black on a transparent canvas (for light section backgrounds). */
  blackStreaks?: boolean;
};
...
void mainImage(out vec4 o, vec2 C) {
  ...
  for (int m = 0; m < 16; m++) {
    ...
    float amt = dot(sm, vec2(exp(tail * Pp.y), 3.0)) * weight;
    if (uBlackStreaks > 0.5) {
      streakMask += amt;
    } else {
      O.rgb += amt * col;
    }
  ...
  if (uBlackStreaks > 0.5) {
    streakMask = clamp(streakMask * uGlow, 0.0, 1.0);
    o = vec4(0.0, 0.0, 0.0, uOpacity * smoothstep(0.0, 0.08, streakMask));
    return;
  }

  vec3 colr = sqrt(tanhv(max(O.rgb * uGlow - vec3(0.04, 0.08, 0.02), 0.0)));
  // Keep empty areas transparent so the host section background shows through.
  float lum = max(max(colr.r, colr.g), colr.b);
  o = vec4(colr, uOpacity * smoothstep(0.0, 0.08, lum));
}
...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      iResolution: { value: [gl.drawingBufferWidth, gl.drawingBufferHeight, 1] as RGB },
      ...
      uBlackStreaks: { value: blackStreaks ? 1 : 0 },
    };
